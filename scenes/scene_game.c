...
static void update_moisture(float dt)
{
    ...
    {
    case WEATHER_TAG_CLEAR:
        basePerMin = 10.f;
        break;
    case WEATHER_TAG_RAIN:
        basePerMin = 5.f;
        break;
    case WEATHER_TAG_CLOUDY:
        basePerMin = 8.f;
        break;
    case WEATHER_TAG_SNOW:
        basePerMin = 6.f;
        break; // ★ 눈: 추측값
    default:
        basePerMin = 8.f;
    ...
    update_status();
    if (s_status.moisture < 0.f)
        s_status.moisture = 0.f;
}
...
static void update_humidity(float dt)
{
    ...
    if (s_room_humidity < 0.f)
        s_room_humidity = 0.f;
    if (s_room_humidity > 100.f)
        s_room_humidity = 100.f;
...
static void update_happiness(float dt)
{
    ...
    if (s_status.happiness < 0.f)
        s_status.happiness = 0.f;
...
static void update_nutrition(float dt)
{
    ...
    if (s_status.nutrition < 0.f)
        s_status.nutrition = 0.f;
...
static void update_weather_event(float dt)
{
    ...
    if (s_activeWeatherEvent == 0)
    {
        ...
        if (r < p)
        {
            ...
            if (weather < 2)
                s_activeWeatherEvent = WEATHER_TAG_RAIN;
            else if (weather > 1 && weather < 4)
                s_activeWeatherEvent = WEATHER_TAG_CLOUDY;
            else if (weather > 3 && weather < 6)
                s_activeWeatherEvent = WEATHER_TAG_SNOW;
            else if (weather > 5 && weather < 8)
                s_activeWeatherEvent = WEATHER_TAG_CLEAR;
...
static void on_window(void *ud)
{
    ...
    if (s_plant)
        log_window(s_plant->id, s_windowOpen);
}
/*
...
static void load_background_animation(void)
{
    ...
    int atlasW = 0, atlasH = 0;
    if (s_bgAtlas)
        SDL_QueryTexture(s_bgAtlas, NULL, NULL, &atlasW, &atlasH);
    ...
    if (!atlasSurface)
    {
        SDL_Log("[ANIM] fallback surface load fail: %s", IMG_GetError());
        s_bgFrameCount = 0;
        return;
    }
    ...
    for (int i = 0; i < s_bgFrameCount; ++i)
    {
        ...
        if (!frameSurface)
        {
            SDL_Log("[ANIM] create frame surface fail: %s", SDL_GetError());
            continue;
        }
        ...
        if (!tex)
        {
            SDL_Log("[ANIM] create texture fail: %s", SDL_GetError());
            continue;
        }
        fr->texture = tex;
    }
    ...
    {
        if (!s_bgFrames[read].texture)
            continue;
        if (write != read)
            s_bgFrames[write] = s_bgFrames[read];
        ++write;
    }
...
static void init(void *arg)
{
    ...
    if (!s_pot)
    {
        ...
        if (!s_pot)
            SDL_Log("GAMEPLAY: load pot.png fail: %s", IMG_GetError());
    }
    ...
    if (!s_bgDay)
    {
        ...
        if (!s_bgDay)
            SDL_Log("GAMEPLAY: load bg_day.png fail: %s", IMG_GetError());
    }
    ...
    if (!s_bgNight)
    {
        ...
        if (!s_bgNight)
            SDL_Log("GAMEPLAY: load bg_night.png fail: %s", IMG_GetError());
    }
    ...
    if (!s_bgSunrise)
    {
        ...
        if (!s_bgSunrise)
            SDL_Log("GAMEPLAY: load bg_sunrise.png fail: %s", IMG_GetError());
    }
    ...
    if (!s_bgTexture)
    {
        ...
        if (!s_bgTexture)
            SDL_Log("GAMEPLAY: load select_Background.png fail: %s", IMG_GetError());
    }
    ...
    if (!s_backIcon)
    {
        ...
        if (!s_backIcon)
            SDL_Log("GAMEPLAY: load I_exit.png fail: %s", IMG_GetError());
    }
    if (!s_water)
    {
        s_water = IMG_LoadTexture(G_Renderer, ASSETS_IMAGES_DIR "I_water.png");
        if (!s_water)
            SDL_Log("GAMEPLAY: load I_water.png fail: %s", IMG_GetError());
    }
    if (!s_window)
    {
        s_window = IMG_LoadTexture(G_Renderer, ASSETS_IMAGES_DIR "I_window.png");
        if (!s_window)
            SDL_Log("GAMEPLAY: load I_window.png fail: %s", IMG_GetError());
    }
    ...
    if (!s_nobug)
    {
        ...
        if (!s_nobug)
            SDL_Log("GAMEPLAY: load I_nobug.png fail: %s", IMG_GetError());
        else
            SDL_Log("loading success");
    }
    if (!s_nogom)
    {
        s_nogom = IMG_LoadTexture(G_Renderer, ASSETS_IMAGES_DIR "I_nogom.png");
        if (!s_nogom)
            SDL_Log("GAMEPLAY: load I_nogom.png fail: %s", IMG_GetError());
        else
            SDL_Log("loading success");
    }
    if (!s_ifhot)
    {
        ...
        if (!s_ifhot)
            SDL_Log("GAMEPLAY: load I_ifhot.png fail: %s", IMG_GetError());
        else
            SDL_Log("loading success");
    }
    if (!s_ifcold)
    {
        ...
        if (!s_ifcold)
            SDL_Log("GAMEPLAY: load I_ifcold.png fail: %s", IMG_GetError());
        else
            SDL_Log("loading success");
    }
    if (!s_lamp)
    ...
    if (!s_lamp_hotbar)
    {
        ...
        if (!s_biryo)
            SDL_Log("GAMEPLAY: load lamp_hotbar.png fail: %s", IMG_GetError());
        else
            SDL_Log("hotbar loading success");
    }
    if (!s_lamp_levelup)
    {
        s_lamp_levelup = IMG_LoadTexture(G_Renderer, ASSETS_IMAGES_DIR "upper.png");
        if (!s_lamp_levelup)
            SDL_Log("GAMEPLAY: load upper.png fail: %s", IMG_GetError());
        else
            SDL_Log("upper loading success");
    }
    if (!s_lamp_leveldown)
    {
        s_lamp_leveldown = IMG_LoadTexture(G_Renderer, ASSETS_IMAGES_DIR "lower.png");
        if (!s_lamp_leveldown)
            SDL_Log("GAMEPLAY: load lower.png fail: %s", IMG_GetError());
        else
            SDL_Log("lower loading success");
    }
    if (!s_lamp_leveldown)
    {
        s_lamp_leveldown = IMG_LoadTexture(G_Renderer, ASSETS_IMAGES_DIR "lower.png");
...
static void handle(SDL_Event *e)
{
    ...
    if (e->type == SDL_KEYDOWN)
    {
        ...
        {
            scene_switch_fade(SCENE_MAINMENU, 0.2f, 0.4f);
            return;
        }
...
static void cleanup(void)
{
    ...
    if (s_water)
    {
        SDL_DestroyTexture(s_water);
        s_water = NULL;
    }
    if (s_window)
    {
        SDL_DestroyTexture(s_window);
        s_window = NULL;
    }
    if (s_nobug)
    {
        SDL_DestroyTexture(s_nobug);
        s_nobug = NULL;
    }
    if (s_nogom)
    {
        SDL_DestroyTexture(s_nogom);
        s_nogom = NULL;
    }
    if (s_ifhot)
    {
        SDL_DestroyTexture(s_ifhot);
        s_ifhot = NULL;
    }
    if (s_ifcold)
    {
        SDL_DestroyTexture(s_ifcold);
        s_ifcold = NULL;
    }
    if (s_lamp)
    {
        SDL_DestroyTexture(s_lamp);
        s_lamp = NULL;
    }
    if (s_biryo)
    {
        SDL_DestroyTexture(s_biryo);
        s_biryo = NULL;
    }
    if (s_lamp_hotbar)
    {
        SDL_DestroyTexture(s_lamp_hotbar);
        s_lamp_hotbar = NULL;
    }
    if (s_lamp_levelup)
    {
        SDL_DestroyTexture(s_lamp_levelup);
        s_lamp_levelup = NULL;
    }
    if (s_lamp_leveldown)
    {
        SDL_DestroyTexture(s_lamp_leveldown);
        s_lamp_leveldown = NULL;
    }

    if (s_weatherClouds)
    {
        SDL_DestroyTexture(s_weatherClouds);
        s_weatherClouds = NULL;
    }
    if (s_weatherRain)
    {
        SDL_DestroyTexture(s_weatherRain);
        s_weatherRain = NULL;
    }
    if (s_weatherSnow)
    {
        SDL_DestroyTexture(s_weatherSnow);
        s_weatherSnow = NULL;
    }
